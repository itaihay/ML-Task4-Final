...
from sklearn.model_selection import RandomizedSearchCV, StratifiedKFold, train_test_split, ParameterGrid
...
for dataset_name in os.listdir(DATASETS_PATH):
    ...
    for train_index, test_index in kf.split(X, y):
        ...
        for grid_values in ParameterGrid({'train_split_size': TRAIN_SPLIT_SIZES, 'prob_limit': PROB_LIMITS}):
            ...
            prob_limit = grid_values['prob_limit']
            ...
            prob_indices = prob.max(axis=1) > prob_limit
            y_train_pseudo[prob_indices] = pred[prob_indices]

            X_train = pd.concat([X_train_real, X_train_pseudo[prob_indices]])
            y_train = np.append(y_train_real, y_train_pseudo[prob_indices])
